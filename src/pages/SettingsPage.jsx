...
export default function SettingsPage() {
  ...
  const [ipOctets, setIpOctets] = useState(["", "", "", ""]);
  ...
  useEffect(() => {
    ...
    const savedIp = localStorage.getItem("serverIp") || "192.168.1.1";
    const octets = savedIp.split(".");
    setIpOctets([
      octets[0] || "",
      octets[1] || "",
      octets[2] || "",
  ...
  const handleIpOctetChange = (index, value) => {
    ...
    if (/^\d*$/.test(value) && value.length <= 3) {
      ...
      // Auto focus next input if current has 3 characters
      if (value.length === 3 && index < 3) {
        document.getElementById(`ip-octet-${index + 1}`)?.focus();
      }
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-slate-50">
      ...
      <div className="max-w-md mx-auto p-6 space-y-6">
        ...
        >
                ...
                {ipOctets.map((octet, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <input
                      ...
                      value={octet}
                      onChange={(e) =>
                        handleIpOctetChange(index, e.target.value)
                      }
                      className="w-full px-3 py-3 text-center border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all font-mono"
          ...
          <button
            type="submit"
            className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-medium hover:bg-blue-500 transition-all duration-200 flex items-center justify-center space-x-2"
          >
